...
export const BookDetailsPage = () => {
    ...
    useEffect(() => {
        const fetchBook = async () => {
            try {
                ...
                const reviews = await reviewService.getReviewsByBookId(id, 0, 10);
                setBookReviews(reviews);
                setHasMore(reviews.page.number < reviews.page.totalPages - 1);
            } catch {
    ...
    const handleLoadMore = async () => {
        ...
        const more = await reviewService.getReviewsByBookId(id, nextPage, 10);
        setBookReviews(r => ({...r, content: [...r.content, ...more.content]}));
        setReviewPage(nextPage);
        setHasMore(more.page.number < more.page.totalPages - 1);
    };
    ...
    const hasUserReviewed = bookReviews?.content?.some(r => r.username === user?.username);
        ...
        <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
                    ...
                    <div className="flex-1 min-w-0">
                        ...
                        <div className="flex flex-wrap gap-2 mb-4">
                            ...
                            {book.publicationDate && (
                                <span
                                    className="text-xs bg-gray-100 text-gray-500 px-2 py-1 rounded-full">{book.publicationDate}
                                </span>
                            )}
                            <span
                                className="text-xs bg-gray-100 text-gray-500 px-2 py-1 rounded-full">{book.genre}
                            </span>
                        ...
                        {isAdminOrMod && book.status === 'PENDING' && (
                            <div className="flex gap-2 mt-4">
                                <button
                                    onClick={() => bookService.changeStatus(id, 'APPROVED').then(() => alert('Approved'))}
                                    className="text-xs px-3 py-1.5 bg-green-100 text-green-700 rounded-lg hover:bg-green-200 font-medium transition-colors">
                                    Approve
                                </button>
                                <button
                                    onClick={() => bookService.changeStatus(id, 'REJECTED').then(() => alert('Rejected'))}
            ...
            <div className="mb-6">
                <h2 className="text-lg font-bold text-gray-900 mb-3">
                    Reviews
                    {bookReviews?.page?.totalElements > 0 && (
                        <span
                            className="ml-2 text-sm font-normal text-gray-400">({bookReviews.page.totalElements})</span>
                    )}
                </h2>
                ...
                <div className="space-y-3">
                    ...
                    {hasMore && (
                        <button
                            ...
                            className="w-full mt-3 py-2 text-sm text-gray-400 border border-gray-100 rounded-xl hover:bg-gray-50 transition-colors">
                            Load more reviews &rsaquo;
                        </button>
                    )}
                </div>
            </div>
            ...
            {user ? (
                hasUserReviewed ? (
                    <div
                        className="bg-gray-50 border border-gray-200 rounded-2xl p-6 text-center text-sm text-gray-400">
